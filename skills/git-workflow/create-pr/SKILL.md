...
Read the `## Git Workflow Config` section from the project documentation for configuration. If not present, use defaults:
...
1. **Read config**: Check project documentation for a `## Git Workflow Config` section and extract the target branch. Default to `staging` if not found.
